...
namespace services.Services
{
    ...
    public class CalculatorService : ICalculatorService
    {
        ...
        private static List<PayoffStep> SimulatePayoff(List<domain.Entities.Debt> orderedDebts, decimal monthlyPayment)
        {
            ...
            var debits = orderedDebts.ToDictionary(d => d.Id, d => d.MonthlyPayment);
            var month = 0;
            ...
            while (balances.Values.Any(b => b > 0))
            {
                ...
                // 1) Pay minimum on every active debt
                foreach (var debt in orderedDebts)
                {
                    if (balances[debt.Id] <= 0) continue;
                    if (remaining <= 0) break;

                    var min = Math.Min(debits[debt.Id], balances[debt.Id]);
                    var pmt = Math.Min(remaining, min);
                    if (pmt <= 0) continue;

                    balances[debt.Id] -= pmt;
                    remaining -= pmt;
                ...
                // 2) Put ALL extra toward active debts in order (target first, cascade when paid off)
                foreach (var debt in orderedDebts)
                {
                    if (balances[debt.Id] <= 0) continue;
                    if (remaining <= 0) break;

                    var extra = Math.Min(remaining, balances[debt.Id]);
